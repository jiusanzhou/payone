...
interface BannerPageComponent extends React.FC<BannerPageProps> {
    noHeader?: boolean
    fullBackground?: string
    powered?: boolean
}

const _emptyObject: BannerData = {}

const BannerPage: BannerPageComponent = ({ isPreview, xdata }) => {
    const router = useRouter()
    const { code, ...args } = router.query as { code?: string; [key: string]: string | string[] | undefined }
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            let channelsData: Record<string, string> = {}
            ...
            const mergedData = { ...xdataStable, ...channelsData, ...args } as BannerData
            setData(mergedData)

            const activeChannels = (channels as Channel[]).filter((c) => !c.disable && mergedData[c.name])
        ...
        loadData()
    }, [code, router.isReady, isPreview, loaded, xdataStable, args])
        ...
        return (
            <div className="flex-1 flex items-center justify-center">
                ...
                <div className="bg-white/95 backdrop-blur rounded-3xl shadow-2xl p-8 md:p-12 max-w-md w-full text-center">
                    ...
                    <a
                        href="/"
                        className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-full font-medium shadow-lg hover:shadow-xl hover:scale-105 transition-all"
                    >
    ...
    const activeChannels = (channels as Channel[]).filter((c) => !c.disable && data[c.name])
                    ...
                    <div className="flex flex-col md:flex-row items-center gap-8">
                        <div className="flex-shrink-0">
                            <div className="bg-white rounded-2xl p-3 shadow-lg ring-1 ring-gray-100">
                                {qrcode && <img src={qrcode} alt="收款二维码" className="w-40 h-40 md:w-48 md:h-48" />}
                            </div>
                        </div>

                        <div className="flex-1 text-center md:text-left">
                            <h1 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">{data._title || '支持我们'}</h1>
                            {data._excerpt && <p className="text-gray-500 mb-4">{data._excerpt}</p>}
                            ...
                            {data._subtitle && <p className="text-gray-400 mt-4 text-sm">{data._subtitle}</p>}
                        </div>
                    </div>
                </div>
            </div>
